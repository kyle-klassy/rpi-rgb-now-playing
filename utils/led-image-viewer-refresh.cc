...
volatile bool refresh_received = false;
...
void DisplayAnimation(const FileInfo *file,
  ...
  for (int k = 0;
       ...
       ++k) {
    uint32_t delay_us = 0;
    while (!refresh_received && !interrupt_received && GetTimeInMillis() <= end_time_ms
...
int main(int argc, char *argv[]) {
  ...
  do {
    ...
    for (size_t i = 0; i < file_imgs.size() && !interrupt_received && !refresh_received; ++i) {
      DisplayAnimation(file_imgs[i], matrix, offscreen_canvas);
    }
  } while (do_forever && !interrupt_received && !refresh_received);
  ...
  if (refresh_received) {
    fprintf(stderr, "Caught SIGALRM, refreshing image\n");
